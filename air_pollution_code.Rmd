...
wf_fit_PM$fit$variable.importance
```
...
### Step 5.2: Fit workflow with cross validation (not done)
...
```{r}
pred_products <- predict(prod_comp_wflow_fit, new_data = train_pm)


yardstick::accuracy(train_pm, 
                truth = product, estimate = pred_products$.pred_class)

count(train_pm, product)
...
```{r}
train_pm <- training(split_complaints)
head(train_pm)
count(train_pm, product)

test_pm <-testing(split_complaints)
head(test_pm)
count(test_pm, product)
```

```{r}
set.seed(1234)
vfold_complaints <- rsample::vfold_cv(data = train_pm, v = 4)
vfold_complaints
...
```{r}
prod_comp_wflow_fit <- parsnip::fit(prod_comp_wflow, data = train_pm)
